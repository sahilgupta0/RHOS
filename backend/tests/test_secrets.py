...
def test_secrets_provider_gcp_success():
    ...
    mock_response.payload.data = (
        b'{"jwt_secret_key": "gcp-jwt-secret", "gemini_api_key": "gcp-gemini"}'
    )
    ...
    with patch.dict(
        ...
        mock_client.access_secret_version.assert_called_once_with(
            request={
                "name": "projects/mock-gcp-project/secrets/mock-secrets/versions/latest"
            }
        )
...
def test_get_settings_injects_secrets():
    ...
    with patch.dict(os.environ, {"SECRET_PROVIDER": "vault"}), patch(
        "app.core.secrets_manager.secrets_provider.fetch_secrets",
        ...
        assert (
            ...
            == "injected-jwt-secret-key-at-least-32-chars"
        )
