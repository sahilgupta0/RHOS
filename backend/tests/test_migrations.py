...
async def test_run_upgrades_no_pending():
    ...
    with patch("os.listdir") as mock_list, patch("os.path.exists", return_value=True), patch("app.core.migration_runner.get_mongodb_db", return_value=mock_db):
...
async def test_run_upgrades_success():
    ...
    with patch("os.listdir") as mock_list, \
         ...
         patch.object(runner, "_load_migration_module", return_value=mock_migration_module):
