...
class SecretsProvider:
    ...
    def fetch_secrets(self) -> dict[str, Any]:
        ...
        if self.provider == "vault":
            ...
            logger.info("Using local environment settings. No external secrets fetched.")
            return {}
    ...
    def _fetch_from_vault(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error("Failed to fetch secrets from Vault: %s. Falling back to local env.", e)
            return {}
    ...
    def _fetch_from_gcp(self) -> dict[str, Any]:
        ...
        if not project_id:
            logger.warning("GCP_PROJECT_ID environment variable is missing. Cannot fetch GCP secrets.")
            return {}
        ...
        try:
            ...
            logger.info("Successfully fetched %d secrets from GCP Secret Manager", len(secrets))
            return secrets
        except ImportError:
            logger.warning("google-cloud-secret-manager library is not installed. Cannot use GCP provider.")
            return {}
        except Exception as e:
